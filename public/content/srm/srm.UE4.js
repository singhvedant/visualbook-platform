...
console.log("Emscripten version: E:/UE_4.22/Engine/Extras/ThirdPartyNotUE/emsdk/emscripten/1.38.20");
console.log("Emscripten configuration: E:/UE_4.22/Engine/Intermediate/Build/HTML5/.emscripten");
...
var Module = {
	...
	UE4_indexedDBName: 'UE4_assetDatabase_srm', // this should be an ascii ID string without special characters that is unique to the project that is being packaged
	UE4_indexedDBVersion: 202201090826, // Bump this number to invalidate existing IDB storages in browsers.
};
...
Module.arguments = ['../../../srm/srm.uproject','-stdout',];
...
$(document).ready(function() {
	...
	function withIndexedDB(db) {
		...
		// ----------------------------------------
		// MORE JS
		var dataJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('srm.data.js'));
...
/* // The following code would download and store the .data file as a Blob, which should be more efficient than loading an ArrayBuffer. However that seems to be buggy, so avoid it for now.
			fetchOrDownloadAndStore(db, Module.locateFile('srm.data')).then(function(dataBlob) {
				return readBlobToArrayBuffer(dataBlob).then(function(dataArrayBuffer) {
					Module['preloadedPackages'] = {};
					Module['preloadedPackages'][Module.locateFile('srm.data')] = dataArrayBuffer;
...
// Instead as a fallback, download as ArrayBuffer. (TODO: Figure out the bugs with the above, and switch to using that one instead)
			fetchOrDownloadAndStore(db, Module.locateFile('srm.data'), 'arraybuffer').then(function(dataArrayBuffer) {
				Module['preloadedPackages'] = {};
				Module['preloadedPackages'][Module.locateFile('srm.data')] = dataArrayBuffer;
				return dataJsDownload.then(addScriptToDom);
			});
